...
DBCheck 是一款开源、跨平台的数据库自动化健康巡检工具，支持 MySQL、PostgreSQL、Oracle、SQL Server、达梦 DM8 及 TiDB 六种主流关系型数据库。工具通过执行预定义的 SQL 检查项与系统资源采集，自动生成格式规范的 Microsoft Word 巡检报告，并提供历史趋势分析、AI 智能诊断、配置基线合规检查、索引健康分析、慢查询深度分析、服务器巡检、分享链接等高级功能。DBCheck 旨在将 DBA 从重复、耗时的手工巡检工作中解放出来，提升数据库运维效率与风险发现能力。
> 官方网站：https://dbcheck.top

> Language: [English](./README.md) | [中文](./README_zh.md)

[![Build Status](https://img.shields.io/badge/build-passing-brightgreen.svg)](https://dbcheck.top)
[![Version](https://img.shields.io/badge/版本-v2.4.7-blue.svg)]()
...
| 接口 | 方法 | 说明 |
|------|------|------|
| `/api/server_inspect_share` | POST | 创建服务器巡检分享链接 |
| `/api/db_inspect_share` | POST | 创建数据库巡检分享链接 |
| `/share/<share_id>` | GET | 查看分享报告（独立页面）|
| `/api/share/<share_id>` | GET | 获取分享数据 |
| `/api/share/<share_id>` | DELETE | 删除分享链接 |
| `/api/shares` | GET | 获取所有分享列表 |
...
> 支持六种主流关系型数据库的全面巡检，覆盖 130+ 条增强规则。
...
> 深度锁分析，结构化报告输出——阻塞链可视化、死锁统计、长事务检测、可执行的修复脚本——现已覆盖全部六种数据库引擎。

#### 诊断维度

| 章节 | 关注点 | 关键指标 |
|------|--------|----------|
| **4.1 锁阻塞链** | 识别多层等待链中的根阻塞源 | 阻塞源 SID / 锁类型 / 锁对象名。严重分档：≥60秒（严重），10–60秒（警告），<10秒（提示）。`fix_sql` 使用正确 SERIAL# 精准终止会话 |
| **4.2 死锁统计** | 累计死锁次数与 trace 分析 | 汇总 `v$sysstat` 死锁计数器，当前等待会话 Top-10，trace 文件分析指南 |
| **4.3 长事务检测** | 检测超过 60 秒阈值的事务 | Undo 块占用、ORA-01555 风险评估、附带回滚估算的终止脚本 |

#### 各数据库覆盖

| 数据库 | 新增 SQL 模板 | 锁分析维度 | 因果模板 |
|--------|:------------:|-----------|:--------:|
| **MySQL** | +4（InnoDB 锁链 / 死锁 / 长事务 / 锁类型统计） | 5维（阻塞者、被阻塞者、锁类型、持续时长、对象） | 6 |
| **PostgreSQL** | +4（阻塞链 / 死锁 / 长事务 / 锁类型分布） | 5维（阻塞 PID、被阻塞 PID、锁模式、持续时长、关系） | 5 |
| **Oracle** | +3（v$lock×v$session 联查阻塞会话 / 死锁统计 / 长事务） | 5维（SID、SERIAL#、锁类型、对象、秒数） | 5 |
| **DM8** | +3（V$LOCK 分析 / V$TRXWAIT 链 / V$TRX 长事务） | 5维（TRX_ID、LTYPE、LMODE、BLOCKED、持续时长） | 5 |
| **SQL Server** | 原生集成（sys.dm_exec_requests 阻塞链 / deadlock_xml / 长运行会话） | 5维 | 4 |
| **TiDB** | 原生集成（CLUSTER_PROCESSLIST / DEADLOCKS / TIDB_TRX） | 5维 | 4 |

> **说明**：SQL Server 和 TiDB 的锁诊断通过系统 DMV / Cluster 表原生集成，无需额外 SQL 模板。全部六种引擎均在 Word 报告中输出结构化锁分析章节。
...
| `POST` | `/api/server_inspect_share` | 创建服务器巡检分享链接 |
| `POST` | `/api/db_inspect_share` | 创建数据库巡检分享链接 |
| `GET` | `/share/<share_id>` | 查看分享报告（独立页面）|
| `GET` | `/api/share/<share_id>` | 获取分享数据 |
| `DELETE` | `/api/share/<share_id>` | 删除分享链接 |
| `GET` | `/api/shares` | 获取所有分享列表 |
