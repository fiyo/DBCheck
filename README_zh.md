...
DBCheck 是一款开源、跨平台的数据库自动化健康巡检工具，支持 MySQL、PostgreSQL、Oracle、SQL Server、达梦 DM8、TiDB、IvorySQL 及 YashanDB 崖山八种主流关系型数据库。工具通过执行预定义的 SQL 检查项与系统资源采集，自动生成格式规范的 Microsoft Word 巡检报告，并提供 SQL 编辑器、远程终端、巡检配置管理、基线配置管理、历史趋势分析、AI 智能诊断、索引健康分析、慢查询深度分析、服务器巡检、分享链接等高级功能。DBCheck 旨在将 DBA 从重复、耗时的手工巡检工作中解放出来，提升数据库运维效率与风险发现能力。
...
| 🔍 160+ 条增强规则 | 覆盖八种数据库全维度风险检测（MySQL 35+条 / PG 27+条 / Oracle 20+条 / SQL Server 15+条 / DM8 16+条 / TiDB 18+条 / IvorySQL 27+条 / YashanDB 15+条）—包括 28 条慢查询深度分析规则 |
...
| 特性 | 说明 |
|------|------|
| 多数据库支持 | MySQL / PostgreSQL / Oracle / SQL Server / DM8 / TiDB / IvorySQL / YashanDB |
...
| 数据库类型识别 | 根据数据源名称自动识别数据库类型，显示彩色标签（MySQL=蓝、PG=紫、TiDB=橙、Oracle=红、DM8=绿） |
...
> 支持八种主流关系型数据库的全面巡检，覆盖 160+ 条增强规则（含 IvorySQL 27+ 条、YashanDB 15+ 条）。


| 维度 | MySQL | PostgreSQL | Oracle | SQL Server | DM8 | TiDB | IvorySQL | YashanDB |
|------|:-----:|:----------:|:------:|:-----------:|:---:|:----:|:---------:|
| 基本信息（版本/实例/数据库） | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |
| 会话与连接状态 | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |
| 内存与缓存配置 | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |
...
> 深度锁分析，结构化报告输出—阻塞链可视化、死锁统计、长事务检测、可执行的修复脚本—现已覆盖全部八种数据库引擎。
...
> **说明**：SQL Server 和 TiDB 的锁诊断通过系统 DMV / Cluster 表原生集成，无需额外 SQL 模板。全部八种引擎均在 Word 报告中输出结构化锁分析章节。
...
| 2 | 选择数据库类型（🐬 MySQL / 🐘 PostgreSQL / 🔴 Oracle / 🟠 SQL Server / 🟡 DM8 / 🐬 TiDB / 🐘 IvorySQL / 🔵 YashanDB） |
