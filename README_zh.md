...
| 数据库 | 新增 SQL 模板 |
|--------|:------------:|-----------|:--------:|
| **MySQL** | +4（InnoDB 锁链 / 死锁 / 长事务 / 锁类型统计） |
| **PostgreSQL** | +4（阻塞链 / 死锁 / 长事务 / 锁类型分布） |
| **Oracle** | +3（v$lock×v$session 联查阻塞会话 / 死锁统计 / 长事务） |
| **DM8** | +3（V$LOCK 分析 / V$TRXWAIT 链 / V$TRX 长事务） |
| **SQL Server** | 原生集成（sys.dm_exec_requests 阻塞链 / deadlock_xml / 长运行会话） |
| **TiDB** | 原生集成（CLUSTER_PROCESSLIST / DEADLOCKS / TIDB_TRX） |
| **IvorySQL** | +4（基于 PG 协议，复用 PG 锁分析逻辑） |
