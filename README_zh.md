...
[![Version](https://img.shields.io/badge/版本-v26.7.19.1-blue.svg)]()
...
DBCheck 同时提供面向中大型生产环境的**专业版**，在"发现风险"之上进一步回答"为什么"和"先做什么"，适合需要深度可观测与根因定位的场景。
...
专业版专属亮点：
- **协同诊断中枢** —— 五位诊断专员（运行监控哨兵、深度巡检分析专员、根因定位分析专员、SQL 治理专员、锁等待分析专员）在同一共享上下文（黑板）上协同，经 SSE 流式展示进度，输出根因推断与可执行、按代价排序的处置方案。
- **eBPF 内核级宿主采集** —— 块设备服务时间百分位（p50/p95/p99）、按进程 IO 与 CPU 归因；仅 opt-in，失败安全降级为 psutil。
- **SSH 安全宿主采集** —— 无 agent 的 Shell 采集，并发护栏 + 硬超时看门狗 + Fernet 加密凭据。
- **统一可观测视图** —— 宿主指标、数据库指标与巡检风险串联为同一条证据链。

> 社区版聚焦"发现风险"，专业版进一步解释"为什么"与"先做什么"。专业版授权请访问 [dbcheck.top](https://dbcheck.top) 或联系作者。
