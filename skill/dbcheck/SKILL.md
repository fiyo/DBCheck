---
name: dbcheck
description: 执行 MySQL 或 PostgreSQL 数据库健康巡检，内置 16+ 条增强风险分析规则 + AI 大模型诊断建议，一键生成专业 Word 巡检报告。适用于 DBA 和运维人员快速掌握数据库运行状况、排查风险。项目地址：https://github.com/fiyo/DBCheck.git
license: MIT
---
...
| 🤖 **AI 智能诊断** | 调用大模型（DeepSeek / OpenAI / Ollama）生成个性化优化建议 |
| 📈 **历史趋势分析** | 多次巡检数据聚合，生成指标趋势折线图 |
...
- **数据库信息**：各库大小、当前活跃进程
- **安全信息**：数据库用户列表及权限概要
- **风险与建议**：16+ 条增强风险分析规则（等级 / 描述 / 优先级 / 负责人 / 修复 SQL），含修复 SQL 速查小节
- **AI 智能诊断建议**：大模型基于巡检数据生成的个性化优化建议（需在 `ai_config.json` 中配置 AI 后端）
- **报告说明**：使用注意事项
